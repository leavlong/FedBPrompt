...
def main_worker(args):
    ...
    if args.load_checkpoint:
        ...
        for i in range(num_users):
            sub_models[i].load_state_dict(checkpoint['state_dict'], strict=False)
    ...
    for epoch in range(args.epochs):  # number of epochs
        ...
        for index in range(num_users):  # client index
            if args.method == "fedprox":
                w, prompt_local = trainer.train_fedprox(sub_models[index], model, epoch, index , op_type= "sgd")
            elif args.method == "fedpav":
                w, prompt_local = trainer.train_fedpav(sub_models[index], epoch, index,op_type="sgd")  # FedPav mixstyle crossstyle
            elif args.method == "mixstyle":
                w, prompt_local = trainer.train_mixstyle(sub_models[index], epoch, index,op_type="sgd")
            else:
                w, prompt_local = trainer.train_crossstyle(sub_models[index], epoch, index,op_type="sgd")
            w_locals.append(w)
        ...
        if args.only_update_prompt:
            ...
            for i in range(num_users):
                sub_models[i].load_state_dict(prompt_global, strict=False)
        else:
            model.load_state_dict(w_global)  # the averaged model
            for i in range(num_users):
                sub_models[i].load_state_dict(w_global, strict=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'fedpav_logs/cuhk03_official'))
