...
class TransReID(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3,
        ...
        self.prompt_group_config = {'upper': 17, 'middle':17, 'lower': 16, 'shared': 0}
...
def vit_base_patch16_224_TransReID_Prompt_PAT(img_size=(256, 128), stride_size=16, drop_rate=0.0,
...
# ====================================================================================
#  新增模块：TransReID_Voter - 实现了五区域划分与特征投票机制
# ====================================================================================

class TransReID_Voter(TransReID):
    def __init__(self, region_mode='3part', fusion_method='voting', *args, **kwargs):
        self.region_mode = region_mode
        self.fusion_method = fusion_method
        print(f"Initializing TransReID_Voter: Mode=[{self.region_mode}], Fusion=[{self.fusion_method}]")
        ...
        #     quality_head_hidden_dim = self.embed_dim // 2
        #     self.quality_head = nn.Sequential(
        #         nn.Linear(self.embed_dim, quality_head_hidden_dim),
        #         nn.ReLU(),
        #         nn.Linear(quality_head_hidden_dim, 1)
        #     )
        # else:
    ...
    def _get_config(self):
        """配置保持不变"""
        if self.region_mode == '2part':
            return {
                'prompt_counts': {'upper': 20, 'lower': 20, 'shared': 10},
                'regions': [
                    {'name': 'upper', 'start_ratio': 0.0, 'end_ratio': 0.5},
                    {'name': 'lower', 'start_ratio': 0.5, 'end_ratio': 1.0}
                ]
            }
        elif self.region_mode == '3part':
            return {
                'prompt_counts': {'upper': 15, 'middle': 15, 'lower': 15, 'shared':5},
                'regions': [
                    {'name': 'upper',  'start_ratio': 0.0,  'end_ratio': 0.5},
                    {'name': 'middle', 'start_ratio': 0.25, 'end_ratio': 0.75},
                    {'name': 'lower',  'start_ratio': 0.5,  'end_ratio': 1.0}
                ]
            }
        elif self.region_mode == '5part':
            return {
                'prompt_counts': {'head': 6, 'upper_torso': 8, 'lower_torso': 8, 'thighs': 8, 'calves': 6, 'shared': 14},
                'regions': [
                    {'name': 'head',        'start_ratio': 0.0,     'end_ratio': 0.1875},
                    {'name': 'upper_torso', 'start_ratio': 0.1875,  'end_ratio': 0.375},
                    {'name': 'lower_torso', 'start_ratio': 0.375,   'end_ratio': 0.5625},
                    {'name': 'thighs',      'start_ratio': 0.5625,  'end_ratio': 0.75},
                    {'name': 'calves',      'start_ratio': 0.75,    'end_ratio': 1.0}
                ]
            }
        else:
    ...
    def _create_prompt_attention_mask(self):
        ...
        self.prompt_group_config = config['prompt_counts']
        
        H, W = self.patch_embed.num_y, self.patch_embed.num_x
        num_patches = H * W
        num_prompts = self.prompt_cfg.NUM_TOKENS
        N = 1 + num_prompts + num_patches 
        ...
        local_prompts_count = sum([self.prompt_group_config[r['name']] for r in config['regions']])
        shared_start_idx = prompt_start_idx + local_prompts_count
        shared_end_idx = shared_start_idx + self.prompt_group_config['shared']
        mask[shared_start_idx:shared_end_idx, :] = True
        mask[:, shared_start_idx:shared_end_idx] = True

        current_prompt_idx = prompt_start_idx
        for region in config['regions']:
            name = region['name']
            count = self.prompt_group_config[name]
            p_start = current_prompt_idx
            p_end = current_prompt_idx + count
            row_start = round(H * region['start_ratio'])
            row_end = round(H * region['end_ratio'])
            if region['end_ratio'] == 1.0: row_end = H
            patch_idx_start = patch_start_idx + row_start * W
            patch_idx_end = patch_start_idx + row_end * W
            mask[p_start:p_end, patch_idx_start:patch_idx_end] = True
            current_prompt_idx += count

        return mask.unsqueeze(0).unsqueeze(0)

    def forward(self, x, return_score = False, **kwargs):
        # --- 步骤 0: 标准特征提取 ---
        B = x.shape[0]
        x = self.patch_embed(x)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        ...
        if self.prompt_cfg.DEEP:
            for i in range(self.depth):
                if i > 0 and i <= len(self.deep_prompt_embeddings):
                    deep_prompt = self.prompt_dropout(self.prompt_proj(
                        self.deep_prompt_embeddings[i - 1]).expand(B, -1, -1))
                    x = torch.cat((x[:, :1, :], deep_prompt, x[:, 1 + self.num_tokens:, :]), dim=1)
                x, _ = self.blocks[i](x, mask=attn_mask)
        else:
        ...
        # 1. 获取全局特征：直接使用 [CLS] token
        #    它始终包含全图信息，是特征的基础
        feat_cls = x[:, 0]

        # 如果模式是 'cls' (基线)，直接返回它，不做任何额外处理
        ...
        local_features_list = []
        for region in config['regions']:
            count = self.prompt_group_config[region['name']]
            region_prompts = x[:, current_idx : current_idx + count, :]
            region_feat = torch.mean(region_prompts, dim=1)
            local_features_list.append(region_feat.unsqueeze(1))
            prompt_outputs.append(region_prompts) 
            current_idx += count
            
        # 局部特征池化: [B, N, D]
        local_features = [torch.mean(p_out, dim=1) for p_out in prompt_outputs]
        stacked_local_features = torch.stack(local_features, dim=1)

        # --- B. 提取全局特征 (Shared/Global Features) ---
        # 此时 current_idx 已经指向了 Shared Prompts 的开始位置
        shared_count = self.prompt_group_config['shared']
        shared_prompts = x[:, current_idx : current_idx + shared_count, :]
        
        # 全局特征池化: [B, D]
        feat_global = torch.mean(shared_prompts, dim=1)
        all_features = torch.cat([feat_cls.unsqueeze(1)] + local_features_list, dim=1)
        # from IPython import embed; embed()
        # 3. 根据方法计算局部特征增量
        feat_local_combined = None
        
        if self.fusion_method == 'mean':
            # 简单平均所有局部特征
            feat_local_combined = torch.mean(stacked_local_features, dim=1)
        elif self.fusion_method == 'cls+local':
            feat_local_combined = all_features
        elif self.fusion_method == 'voting':
            pass
            # # 投票机制加权
            # quality_scores = self.quality_head(stacked_local_features)
            # quality_scores = quality_scores.squeeze(-1) 
            # quality_scores = quality_scores * self.scale 
            # vote_weights = F.softmax(quality_scores, dim=1)
            # temp = vote_weights.cpu.numpy()
            # config = self._get_config
            # region_names = [r['name'] for r in config['regions']]
            # for r_name, w_val in zip(region_names, temp):
            #     print(f"    | {r_name:<12}: {w_val:.4f}")
            
            # max_idx = temp.argmax()
            # print(f"  > Most Important Part: [{region_names[max_idx]}] (Score: {temp[max_idx]:.4f})")
                
            # feat_local_combined = (vote_weights.unsqueeze(-1) * stacked_local_features).sum(dim=1)
        elif self.fusion_method == "mean+cls":
            feat_local_combined = torch.mean(stacked_local_features, dim=1)
            feat_local_combined = feat_local_combined + feat_cls
        elif self.fusion_method == "cls":
            feat_local_combined = feat_cls
        else:
        ...
        if return_score:
            # 1. 计算原始模长 (作为置信度/权重)
            # [B, K]
            feature_norms = torch.norm(stacked_local_features, p=2, dim=-1)
            
            # 2. 对特征进行归一化 (用于计算余弦相似度)
            # [B, K, 768]
            normalized_features = F.normalize(stacked_local_features, p=2, dim=-1)
            return normalized_features, feature_norms
        # 4. 最终融合：[CLS] + [Local Correction]
        #    这里直接相加 (Feature Addition)，类似于残差连接
        return feat_local_combined
        # return feat_cls + feat_local_combined
        # return feat_cls + feat_global

# ====================================================================================
#  如何使用新的模型
# ====================================================================================

def vit_base_patch16_224_TransReID_Voter_Pro(img_size=(256, 128), **kwargs):
    # 1. 获取 args 对象 (它被 build_transformer 放在了 kwargs 里)
    args = kwargs.get('args', None)

    # 2. 优先从 kwargs 里找，找不到再去 args 对象里找，都找不到用默认值
    # 处理 region_mode
    if 'region_mode' in kwargs:
        region_mode = kwargs.pop('region_mode')
    elif args is not None and hasattr(args, 'region_mode'):
        region_mode = args.region_mode
    else:
        region_mode = '5part' # 默认值

    # 处理 fusion_method
    if 'fusion_method' in kwargs:
        fusion_method = kwargs.pop('fusion_method')
    elif args is not None and hasattr(args, 'fusion_method'):
        fusion_method = args.fusion_method
    else:
        fusion_method = 'voting' # 默认值

    # 处理其他参数...
    stride_size = kwargs.pop('stride_size', 16)
