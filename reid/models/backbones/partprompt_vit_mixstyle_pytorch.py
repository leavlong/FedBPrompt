...
class PatchEmbed_overlap(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        self.patch_size = patch_size
        self.num_patches = num_patches
        self.grid_size = (self.num_x , self.num_y)
...
class TransReID(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3,
                 ...
                 local_feature=False, args=None , mixstyle_module = None , prompt_cfg = None):
        super().__init__()
        self.mixstyle = mixstyle_module
        ...
        self.prompt_group_config = {'upper': 15, 'middle': 15, 'lower': 15, 'shared': 5}
    ...
    #     super().train(mode)  # 调用nn.Module的train方法
    #
    #     if mode:
    #         # 冻结骨干网络 (Patch Embedding, CLS token, Pos Embed, Blocks的权重)
    #         # 注意：如果Block内部的Attention模块有Prompt参数，它们仍应是可训练的
    #         # 这里需要更精细的控制，例如遍历所有参数并设置requires_grad
    ...
    #             if hasattr(block.attn, 'k_prompt_embeddings') and block.attn.k_prompt_embeddings is not None:
    #                 print("k_prompt_embeddings has fired!!")
    #                 block.attn.k_prompt_embeddings.requires_grad = True
    #             if hasattr(block.attn, 'v_prompt_embeddings') and block.attn.v_prompt_embeddings is not None and \
    #                     block.attn.prompt_cfg.SHARE_PARAM_KV == False:  # 只有不共享时V才独立
    #                 print("v_prompt_embeddings has fired!!")
    #                 block.attn.v_prompt_embeddings.requires_grad = True
    #
    #         # 确保Prompt相关模块可训练
    #         self.prompt_embeddings.requires_grad = True
    #         if self.prompt_cfg.DEEP and hasattr(self, 'deep_prompt_embeddings'):
    #             self.deep_prompt_embeddings.requires_grad = True
    #         for param in self.prompt_proj.parameters():
    #             param.requires_grad = True
    #
    #         # 剪枝掩码的requires_grad在mask_soft_tokens/pieces中处理
    #     else:  # 评估模式，所有参数都设置为eval
    #         for module in self.children():
    #             module.eval()

    def forward(self, x, **kwargs):
        B = x.shape[0]
        x = self.patch_embed(x)  # 获取patch嵌入
        # 添加CLS token
        ...
        # --- 在这里应用 MixStyle ---
        # 2. 从序列中分离出 patch tokens
        num_prompts = self.prompt_cfg.NUM_TOKENS
        cls_token_part = x[:, :1]
        prompt_tokens_part = x[:, 1: 1 + num_prompts]
        patch_tokens_part = x[:, 1 + num_prompts:]

        B, N_p, D = patch_tokens_part.shape
        H_p, W_p = self.patch_embed.grid_size

        # 验证: N_p 应该等于 H_p * W_p
        if N_p == H_p * W_p:
            feature_map = patch_tokens_part.permute(0, 2, 1).reshape(B, D, H_p, W_p)

            # 2c. 将特征图输入 MixStyle 模块
            mixed_feature_map = self.mixstyle(feature_map)

            # 2d. 将处理后的特征图 "压平" 回序列形状
            mixed_patch_tokens = mixed_feature_map.flatten(2).permute(0, 2, 1)

            # 2e. 重新拼接所有部分
            if prompt_tokens_part is not None:
                x = torch.cat((cls_token_part, prompt_tokens_part, mixed_patch_tokens), dim=1)
            else:
                x = torch.cat((cls_token_part, mixed_patch_tokens), dim=1)
        else:
            # 如果形状不匹配，打印警告并跳过MixStyle，避免程序崩溃
            print(f"Warning: Skipping MixStyle due to shape mismatch. "
                  f"Num patches ({N_p}) != grid size ({H_p}*{W_p}).")
        # --- MixStyle 应用结束 ---

        last_layer_attn = None
        # 注意：这里我们直接使用 self.prompt_attn_mask，它已经被注册为 buffer，会自动移动到正确的 device
        attn_mask = self.prompt_attn_mask

        # --- Part-Prompt Transformer Block 逻辑 ---
        if self.prompt_cfg.DEEP:
            for i in range(self.depth):
...
def vit_base_patch16_224_TransReID_MixStyle_PartPrompt(img_size=(256, 128), stride_size=16, drop_rate=0.0,
                                   attn_drop_rate=0.0, drop_path_rate=0.1, local_feature=False,
                                   args=None, **kwargs):
    mixstyle_module = MixStyle(p=0.5, alpha=0.1, mix='random')
    model = TransReID(
        ...
        local_feature=local_feature, args=args, mixstyle_module = mixstyle_module , **kwargs)
    return model
