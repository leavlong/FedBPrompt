...
class TransReID(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3,
                 ...
                 local_feature=False, args=None , mixstyle_module = None):
        super().__init__()
    ...
    def forward(self, x, **kwargs):
        ...
        # 3. 将 patch tokens "恢复" 成特征图的形状
        # (B, N, D) -> (B, D, N) -> (B, D, H_p, W_p)
        B, N, D = patch_tokens.shape
        # 从patch_embed层获取网格大小 (grid_size)
        H_p, W_p = self.patch_embed.grid_size
        feature_map = patch_tokens.permute(0, 2, 1).reshape(B, D, H_p, W_p)

        # 4. 将 "特征图" 输入 MixStyle 模块
        # MixStyle内部会处理好是否在训练模式下激活
        mixed_feature_map = self.mixstyle(feature_map)

        # 5. 将处理后的特征图 "压平" 回序列形状
        # (B, D, H_p, W_p) -> (B, D, N) -> (B, N, D)
        mixed_patch_tokens = mixed_feature_map.flatten(2).permute(0, 2, 1)

        # 6. 重新拼接CLS token
        if self.cls_token is not None:
            x = torch.cat((cls_token, mixed_patch_tokens), dim=1)
        else:
            x = mixed_patch_tokens
        # --- MixStyle 应用结束 ---
...
def vit_base_patch16_224_TransReID_MixStyle(img_size=(256, 128), stride_size=16, drop_rate=0.0,
