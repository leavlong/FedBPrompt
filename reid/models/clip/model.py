...
@dataclass
class PromptConfig:
    NUM_TOKENS: int = 50  # 提示令牌的数量
    DROPOUT: float = 0.0  # 提示的 Dropout 率
    DEEP: bool = True  # 是否使用深层提示
    PROJECT: int = -1  # 提示的投影维度（-1 表示不投影）
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int,
                 heads: int, output_dim: int, prompt_config=None):
        super().__init__()
        ...
        self.width = width
        # 计算图像块的数量（考虑 stride）
        self.grid_size = (h_resolution, w_resolution)
        self.num_patches = h_resolution * w_resolution
        self.seq_length = self.num_patches + 1
        if prompt_config is not None:
            self.num_tokens = prompt_config.NUM_TOKENS
            self.seq_length += self.num_tokens  # CLS + Prompts + 图像块
        self.positional_embedding = nn.Parameter(self.scale * torch.randn(self.seq_length, width))
        ...
        if prompt_config is not None:
            ...
            self.num_tokens = prompt_config.NUM_TOKENS
            self.prompt_dim = width  # 与 ViT 隐藏层维度一致
            val = math.sqrt(6. / float(3 * reduce(mul, _pair((patch_size, patch_size)), 1) + PromptConfig.PROJECT))
            # 浅层提示（仅输入层）
            self.prompt_embeddings = nn.Parameter(torch.zeros(1, self.num_tokens, self.prompt_dim))
            nn.init.uniform_(self.prompt_embeddings.data, -val, val)  # 小范围初始化

            # 深层提示（每层独立）
            if prompt_config.DEEP:
                total_d_layers = layers - 1  # 除第一层外的层数
                self.deep_prompt_embeddings = nn.Parameter(
                    torch.zeros(total_d_layers, self.num_tokens, self.prompt_dim))
                nn.init.uniform_(self.deep_prompt_embeddings.data, -val, val)

    def set_stage(self, stage, prompt_config):
        self.stage = stage
        if stage == "stage2" and prompt_config is not None:
            self.seq_length += self.num_tokens  # CLS + Prompts + 图像块
            self.positional_embedding = nn.Parameter(
                self.scale * torch.randn(self.seq_length, self.width).cuda())  # 重新初始化位置编码

    def prompt_resize_pos_embed(posemb, posemb_new, h_resolution, w_resolution, num_tokens=0):
        """
        posemb: 原始位置编码 (原始序列长度, hidden_dim)
        posemb_new: 新位置编码的参数张量 (新序列长度, hidden_dim)
        h_resolution, w_resolution: 分块后的网格大小（已考虑 stride）
        num_tokens: 提示令牌的数量
        """
        # 分离 CLS 和图像块位置编码
        ...
        posemb_grid = F.interpolate(
            posemb_grid,
            size=(h_resolution, w_resolution),  # 直接使用分块后的网格大小
            mode='bilinear'
        )
        posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, h_resolution * w_resolution, -1)
        ...
        # 如果有提示令牌，填充随机初始化位置编码
        if num_tokens > 0:
            prompt_posemb = torch.randn(num_tokens, posemb.shape[1], device=posemb.device) * 0.02
            posemb = torch.cat([posemb[:1], prompt_posemb, posemb[1:]], dim=0)
    ...
    def incorporate_prompt(self, x):
        ...
        x = torch.cat([cls_token, prompts, x], dim=1)  # [B, 1 + num_tokens + grid^2, width]
    ...
    def forward_deep_prompt(self, x):
        """处理深层提示的前向传播"""
        B = x.shape[0]
        x = self.incorporate_prompt(x)  # 初始嵌入（含浅层提示）
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # [L, B, D]

        # 逐层处理 Transformer
        for i in range(len(self.transformer.resblocks)):
            if i > 0 and self.prompt_config.DEEP and i <= self.deep_prompt_embeddings.shape[0]:
                # 插入当前层的深层提示
                deep_prompt = self.prompt_dropout(self.deep_prompt_embeddings[i - 1]).expand(B, -1, -1)
                x = torch.cat([
                    x[:1, :, :],  # CLS 令牌
                    deep_prompt.permute(1, 0, 2),  # 深层提示 [num_tokens, B, D]
                    x[1 + self.num_tokens:, :, :]  # 图像块
                ], dim=0)
    ...
    def forward(self, x: torch.Tensor, cv_emb=None, label=None):
        if self.prompt_config is None:
            ...
            if self.prompt_config.DEEP:
                x1, x2 = self.forward_deep_prompt(x)
            else:
                ...
                # 对整个序列做 LayerNorm
            x_all = self.ln_post(x2)  # [B, num_tokens + 1 + grid^2, D]

            # 投影整个序列
            if self.proj is not None:
                xproj = x_all @ self.proj  # [B, seq_len, output_dim]

            # 返回三个完整序列输出
            return x1, x_all, xproj  # 全部 [B, seq_len, D]
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int, PromptConfig=None):
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution, PromptConfig
    )
    if vit:
        if PromptConfig is not None:
            state_dict["visual.positional_embedding"] = prompt_resize_pos_embed(
                state_dict["visual.positional_embedding"], h_resolution, w_resolution, PromptConfig.NUM_TOKENS)
        else:
            state_dict["visual.positional_embedding"] = resize_pos_embed(
                state_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution,
                w_resolution)
    else:  # RN50
...
def prompt_resize_pos_embed(posemb, h_resolution, w_resolution, num_tokens=0):
    """
    posemb: 原始位置编码 (原始序列长度, hidden_dim)
    posemb_new: 新位置编码的参数张量 (新序列长度, hidden_dim)
    h_resolution, w_resolution: 分块后的网格大小（已考虑 stride）
